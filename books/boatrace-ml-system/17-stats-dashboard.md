...
<!-- 図: boatrace-fun.net の /stats/ ページのスクリーンショット。上から「直前通算」の表(予想者 / レース数 / 的中率 / 回収率)、累積回収率の折れ線(破線 = 100%)、その下に「本命予想 — 分析軸別」として 場別 / グレード別 / 買い目点数別 / 本命枠番別 / 配当帯別 / 風速別 の 6 表が 2 列グリッドで並ぶ。n < 20 の行が灰色で「参考」バッジ付きになっている箇所が写るように切り出す -->
